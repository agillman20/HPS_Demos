...
%  number of levels in the HPS method
L = 1:10;
% xp  = a point in box_geom where we want to
%       look at the convergence.
xp = 0.1;
% upt(j) = approximate solution at xp with L(j)
%           discretization points.
upt = zeros(1,length(L));
...
for j =1:length(L)
    ...
    % create a list of all the leaf boxes
    leaf_list = makeleaf_list(NODES);

    %  sweep over leaves to find box that xp lives
    %  in and compute the approximate
    for l = 1:length(leaf_list)
        jbox = leaf_list(l);
        box = NODES{1,jbox};
        % figure out if the
        if (xp>box(1) && xp<=box(2))
            % points in the leaf box
            xx = NODES{10,jbox};
            % create interpolation
            Interp = LOCAL_interpolation_matrix(xp',xx');
            % extract the solution on the leaf box
            uapp = u{2,jbox};
            % create the approximate soluiton at xp
            upt(j) =Interp*uapp;
        end
    end
end
...
% look at the convergence
err = zeros(1,length(L)-1);
for j = 1:length(L)
    if j>1
        err(1,j-1) = abs(upt(j)-upt(j-1))/abs(upt(j));
    end
end

% plot of the N vs error
figure(2)
loglog(Ncheb.^L(1:end-1),err)

% p estimate order
nn = Ncheb.^L(1:end-1);
h = 1./nn;
log(abs(err(1,1:end-1)./err(1,2:end)))
...
src = 1/sqrt(.1*pi)*exp(-(xx-0.875).^2/0.05);
...
kh = 10;
ima = sqrt(-1);
% ubdy = 0*sin(1./(alpha+xx));
% un = -1./(alpha+xx).^2.*cos(1./(alpha+xx));
ubdy = exp(ima*kh*xx);
un = ima*kh*exp(ima*kh*xx);
