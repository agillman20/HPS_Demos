function Helmholtz_nobody

% This code solves variable coefficient Helmholtz problems
% with no body load and Dirichlet boundary conditions.
rng('default')
rng(0)
format long

% This code solves
%  -Laplace*u - diag(kh*kh*(1-b(x)))*u = 0 for x in box_geom
%   where u is the unknown solution, and the function
% b(x) has compact support contained in box_geom
% Dirichlet boundary data is considered.  
% alternative boundary conditions can also be handled.
...
% exact solution to this boundary value problem
ima = sqrt(-1); 
uex = @(x,y) exp(ima*kh*x);

M = [1:4];


soln = zeros(2,length(M));
err  = zeros(2,length(M));
...
yy = NODES{13,1}; % points on the boundary.
eta = kh;
ima = sqrt(-1);
T = -ima*eta*inv(ItI-eye(size(ItI,1)))*(ItI+eye(size(ItI,1))); % Interior DtN Operator

    % this code does not call a subroutine to build the boundary data.
    % Instead the boundary data is created by given Dirichlet boundary
    % data and impedance boundary data is created to pass into the downward
    % sweep
    % create the dirichlet boundary conditions
    udir = uex(yy(1,:),yy(2,:)).';

    % make the homogenous solution info
    uhom = udir;
    uhom_n = T*uhom;

   % incoming impedance boundary data for creating the interior solution 
    bc = uhom_n + ima*eta*(uhom); 

    %%% Downward Solve Sweep
    [u] = downward_sweep(NODES,bc);
    
    ptlist = [0.13;0.13];
    leaf_list = makeleaf_list(NODES);
    soln(1,j) = interp_solution(NODES,leaf_list,u,ptlist,Ncheb);
        fprintf(1,'u_{N+1}(0.1,0.13) = %12.5e\n', soln(1,j))
        fprintf(1,'u_ex(0.13,0.13) = %12.5e\n', uex(ptlist(1),ptlist(2)))
        fprintf(1,'|u_{N+1}(0.13,0.13)-u_ex(0.13,0.13)|= %12.5e\n', abs(uex(ptlist(1),ptlist(2))-soln(1,j)))


        err(1,j) = abs(soln(1,j)-uex(ptlist(1),ptlist(2)));
...
elseif (params(1) == 11)
    kh = params(2);
    A  = -L - diag(kh*kh*(1-b));
elseif (params(1) == 33) % constan coefficient 
     kh = params(2);
    A = -L - diag(kh*kh*b);
...
elseif (params(1) == 33) % Constant coefficient
    b = ones(size(XX1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % 
% % % function [u] = LOCAL_source(xx,params)
% % % 
% % % % this needs to modified depending on the exact solution
% % % % for test problems or with what it is for your problem
% % %     ima = sqrt(-1);
% % %     kh = params(2);
% % %     q = LOCAL_bump(xx(1,:),xx(2,:),params);
% % %     f = exp(ima*kh*xx(1,:).').*sin(xx(2,:).').*(1+kh^2-kh^2*(1-q'));
% % %     u = f ;
% % % 
% % % return
...
elseif (params(1) == 11) % Single Gaussian Bump
    kh = params(2);
    uex = @(x,y) sin(kh*pi*(x+0.5)).*sin(kh*pi*(y+0.5));
    uex_xx = @(x,y) -kh*kh*pi*pi*uex(x,y);
    uex_yy = @(x,y) -kh*kh*pi*pi*uex(x,y);

    q = LOCAL_bump(xx(1,:),xx(2,:),params);
    
    mL = -uex_xx(xx(1,:),xx(2,:))-uex_yy(xx(1,:),xx(2,:));
    u = mL-kh^2*(1-q).*uex(xx(1,:),xx(2,:));
    u = u.';
...
return
% iti_solve_downwardsweep.m

function [u] = downward_sweep(NODES,bc)
...
for ibox = 1:nboxes
    ...
    if isempty(NODES{4,ibox})
        % leaf box
        u{3,ibox} = NODES{24,ibox}*u{2,ibox};
    else
        % for non-leaf boxes make incoming impedance data
        ison1 = NODES{4,ibox}(1);
        ison2 = NODES{4,ibox}(2);
        
        vloc1 = NODES{24,ibox}*u{2,ibox};  % find the solution on the interior
        %                                               edge for ison1
        vloc2 = NODES{25,ibox}*u{2,ibox};  % find the solution on the interior
        %                                               edge for ison2
