function Helmholtz_DIR


% This code solves variable coefficient Helmholtz problems
% with a body load and Dirichlet boundary conditions.
rng('default')
rng(0)
format long
...
for j = 1:length(M)
    ...
    [u,v,~] = iti_solve_upwardsweep(NODES,params,Ncheb);
    h = u{1,1};
    ...
    bc = uhom_n+p_n + ima*eta*(p+uhom);

    %%% Downward Solve Sweep
    [u,~] = iti_solve_downwardsweep(NODES,u,v,bc);
...
function [u,v,t_up] = iti_solve_upwardsweep(NODES,params,Ncheb)
...
for ibox = nboxes:-1:1
    if NODES{5,ibox}==0 % leaf box
        NODES = process_leaf_box(LEAF_info,NODES,ibox,params,Ncheb,eta);
    else
...
function [NODES] =  process_leaf_box(LEAF_info,NODES,ibox,params,Ncheb,eta)
...
function [u,t_down] = iti_solve_downwardsweep(NODES,u,v,bc)
